(function(impress) {
	...
	impress.require = function(moduleName) {
		var lib = null,
			exists = impress.fs.existsSync(impress.dir+'/node_modules/'+moduleName);
		if (!exists) exists = impress.fs.existsSync(impress.dir+'/node_modules/impress/node_modules/'+moduleName);
		if (exists) lib = require(moduleName);
	...
	impress.init = function(callbacks) {
		if (impress.cluster.isMaster && impress.config.startup && impress.config.startup.check) {
			...
			}).on('error', function () { startup(afterInit); });
