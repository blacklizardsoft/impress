(function(impress) {
    ...
    impress.uuid = require('node-uuid');
		...
		application.preprocessConfig = function() {
			...
			if (config.routes) {
				...
				for (var i = 0; i < routes.length; ++i) {
                    ...
                    route.urlRx = new RegExp('^'+route.url.replace(/\(\\\.\*\)/, "(.*)")+'$');
	...
	impress.server.start = function() {
		...
		function startup() {
			...
			//console.log('---startup:'+impress.workerApplicationName);
			if (impress.workerApplicationName) {
				//console.log('---startup:do');
				var application = impress.applications[impress.workerApplicationName];
				console.log('Worker process forked: '+impress.workerApplicationName+' file: '+impress.workerApplicationFile);
				console.dir(impress.workerApplicationClient);
				application.runScript(impress.workerApplicationFile, impress.workerApplicationClient, function() {
					console.log('Worker process exit: '+impress.workerApplicationName);
					process.exit(0);
	...
	var Client = function(req, res) {
		...
		extend(this, {
			...
			access: clone(impress.defaultAccess),
			query: impress.querystring.parse(url.query),
			schema: (!req.connection.server) ? "https" : "http",
			url: url.pathname,
			path: url.pathname,
			ext: fileExt(url.pathname) || 'html',
			slowTime: server.slowTime
		});
		...
		if (sid) {
			if (impress.validateSID(this.application.config, sid)) {
				if (impress.security && this.application.sessions[sid]) {
					...
					if (impress.security) this.user = impress.security.getSessionUser(this.application, sid);
					this.processing();
				} else {
