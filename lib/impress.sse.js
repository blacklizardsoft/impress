(function(impress) {
	...
	impress.sse.connect = function(client) {
		var userId = (client.session && client.logged) ?
            ((typeof client.application.sessions[client.session].userId.toHexString === 'function') ?
                client.application.sessions[client.session].userId.toHexString() : client.application.sessions[client.session].userId) : null;
