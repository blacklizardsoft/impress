(function(impress) {
            ...
            if (user) {
                if (user.emailVerified) {
                    ...
                    if (user.Password == hash) {
                        ...
                        if (user.type) client.application.sessions[client.session].group = user.type;
                ...
                accounts.createNewAccount(db, new_account, function (err, recordId) {
                    if (recordId > 0) {
                        impress.security.getAccountById(db, recordId, function (err, acc) {
                            ...
                            if (acc.type) client.application.sessions[client.session].group = acc.type;
                            client.application.sessions[client.session].sessionModified = true;
                            client.logged = true;
    ...
    impress.security.getAccountByEmail = function(db, email, callback) {
        var query_str =
            'SELECT accounts.*, account_types.Group as type, account_types.Group as `group` '+
            'FROM accounts, account_types '+
            'WHERE accounts.AccountTypeId = account_types.Id and accounts.Email = ?;';
    ...
    impress.security.getAccountById = function(db, accountId, callback) {
        var query_str =
            'SELECT accounts.*, account_types.Group as type, account_types.Group as `group` '+
            'FROM accounts, account_types '+
            'WHERE accounts.AccountTypeId = account_types.Id and accounts.Id = ?;';
    ...
    impress.security.getAccountByToken = function(db, token, callback) {
        var query_str =
            'SELECT accounts.*, account_types.Group as type, account_types.Group as `group` '+
            'FROM accounts, account_types, tokens '+
            'WHERE accounts.AccountTypeId = account_types.Id '+
