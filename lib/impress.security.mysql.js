(function(impress) {
    ...
    impress.security.signIn = function(db, client, callback) {
        impress.security.getAccountByEmail(db, client.fields.Login, function getAccountByEmailCallbackSignIn(err, user) {
            if (user){
                var hash = impress.crypto.createHash('md5').update(client.fields.Password).digest('hex');
                if (user.Password == hash) {
                    ...
                    impress.security.getAccountTypeById(db, user.AccountTypeId, function(err, accType) {
                        if (accType) client.application.users[user.Id].group = accType.Group;
                    });
        ...
        impress.security.getAccountByEmail(db, client.fields.Email, function (err, account) {
            if (!account) {
                var new_account = {
                    ...
                    Password: impress.crypto.createHash('md5').update(client.fields.Password).digest('hex'),
    ...
    impress.security.getAccountByToken = function(db, token, callback) {
        db.queryRow('SELECT accounts.* FROM accounts, tokens WHERE accounts.Id = tokens.accountId and tokens.token = ?;', [token],
            function (err, row) {
                if (callback) callback(err, row);
            }
        );
